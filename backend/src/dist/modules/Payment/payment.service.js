...
const initPayment = (payload) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    while (yield prisma_1.default.payment.findUnique({
        where: {
...
const getMyPaymentsFromDB = (authUser, paginationOptions) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const data = payments.map((payment) => {
        const { gatewayGatewayData } = payment, main = __rest(payment, ["gatewayGatewayData"]);
        return main;
    });
    const meta = {
        page,
...
const getPaymentsFromForManageDB = (filter, paginationOptions) => __awaiter(void 0, void 0, void 0, function* () {
    const { skip, limit, page, orderBy, sortOrder } = (0, paginationHelper_1.calculatePagination)(paginationOptions);
    const andConditions = [];
    const { minAmount, maxAmount, startDate, endDate, status, customerId } = filter;
...
const checkPayment = (query) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        if (!query.token)
            throw new Error();
        const decode = (yield jwtHelpers_1.default.verifyToken(query.token, config_1.default.jwt.payment_secret));
        if (!decode)
        ...
        yield prisma_1.default.$transaction((tx) => __awaiter(void 0, void 0, void 0, function* () {
            switch (query.status) {
                case "SUCCESS":
                    yield order_service_1.default.placeOrderAfterSuccessfulPaymentIntoDB(payment.id, tx);
                ...
                case "CANCELED":
                    ...
                    url = config_1.default.payment.cancel_url;
                    yield order_service_1.default.manageUnsuccessfulOrdersIntoDB("FAILED", payment.order.id, tx);
                    break;
                case "FAILED":
                    yield tx.payment.update({
                        where: {
