...
const Select = ({ options, defaultValue, onChange }: IProps) => {
  ...
  return (
    <div className="relative select-none">
      ...
      >
        <p className=" md:font-semibold text-primary">{selected?.display}</p>
      ...
      {isOpen && (
        <div
          ref={ref}
          className="absolute  top-16 left-2 md:h-60 h-52   overflow-y-auto w-full  rounded-lg no-scrollbar customize-scrollbar z-50 bg-white shadow-2xl  border-primary py-2"
        >
          {options.map((option, index) => (
            ...
            >
              <p className=" md:text-lg text-sm font-medium font-secondary group-hover:text-primary">
                {option.display}
              </p>
            </div>
          ))}
