    ...
    <div>
      ...
      <div className="mt-5">
        <div>
          ...
          <p
            className={`${[EOrderStatus.PLACED, EOrderStatus.IN_TRANSIT].includes(order.status as any) ? "text-primary" : EOrderStatus.DELIVERED === order.status ? "text-green-500" : "text-red-500"} font-medium text-sm  rounded-full md:scale-100 scale-70`}
          >
            Status: {order.status}
          </p>
        </div>
            ...
            <div key={index} className="flex gap-3">
              ...
              <div>
                ...
                <div className="flex items-center gap-2 text-sm">
                  ...
                  <span>
                    Price: <FaBangladeshiTakaSign className=" inline" />
                    {item.price}
                  </span>
                </div>
              </div>
            ...
            <div key={index} className="mt-2">
              ...
              <div>
                {details.values.map((value, index2) => {
                  return (
                    <p key={index2}>
                      <span className="font-medium">{value.name}:</span> <span>{value.value}</span>
                    </p>
                  );
                })}
              </div>
